...
Version: 3.0 - Raspberry Pi 5 - DUAL STORAGE (SQLite + InfluxDB)
Affichage Terminal + Serveur Web + Base de données SQLite + InfluxDB
"""
...
# ============================================================================
# GESTIONNAIRE INFLUXDB
# ============================================================================

class InfluxDBManager:
    """Gestion de la connexion et écriture vers InfluxDB"""
    
    def __init__(self, host: str = 'localhost', port: int = 8086, 
                 database: str = 'surveillance'):
        """
        ...
        try:
            ...
            if {'name': database} not in databases:
                ...
                print(f"✅ Base de données InfluxDB '{database}' créée")
            
            self.client.switch_database(database)
            print(f"✅ InfluxDB connecté: {host}:{port}/{database}")
            
        except Exception as e:
            print(f"⚠️  InfluxDB non disponible: {e}")
            print("   → Les données seront stockées uniquement dans SQLite")
            self.enabled = False
            self.client = None
    
    def ecrire_mesure_environnement(self, mesure: MesureEnvironnement) -> bool:
        """Écrit une mesure environnementale dans InfluxDB"""
        ...
        try:
            json_body = [
                {
                    ...
                    "time": mesure.timestamp,
                    "fields": {
                        "temperature_C": float(mesure.temperature_C),
                        "humidity_pct": float(mesure.humidity_pct),
                        "point_rosee": float(mesure.point_rosee),
                        "indice_chaleur": float(mesure.indice_chaleur)
                    }
                }
            ]
            ...
            print(f"❌ Erreur écriture InfluxDB (environnement): {e}")
            return False
    
    def ecrire_mesure_electrique(self, mesure: MesureElectrique) -> bool:
        """Écrit une mesure électrique dans InfluxDB"""
        if not self.enabled or not self.client:
            return False
        ...
        try:
            json_body = [
                {
                    ...
                    "time": mesure.timestamp,
                    "fields": {
                        "voltage_V": float(mesure.voltage_V),
                        "current_A": float(mesure.current_A),
                        "power_W": float(mesure.power_W),
                        "energy_Wh": float(mesure.energy_Wh),
                        "frequency_Hz": float(mesure.frequency_Hz),
                        "power_factor": float(mesure.power_factor),
                        "alarm": int(mesure.alarm)
                    }
                }
            ]
            self.client.write_points(json_body)
            ...
            print(f"❌ Erreur écriture InfluxDB (électrique): {e}")
            return False
...
class DatabaseManager:
    ...
    def __init__(self, db_path: str = "surveillance.db"):
        ...
        self.lock = Lock()
        self._creer_tables()
        print(f"✅ Base de données SQLite initialisée: {db_path}")
    ...
    def inserer_mesure_environnement(self, mesure: MesureEnvironnement) -> bool:
        ...
        try:
            ...
            print(f"❌ Erreur SQLite (environnement): {e}")
            return False
    
    def inserer_mesure_electrique(self, mesure: MesureElectrique) -> bool:
        ...
        try:
            ...
            print(f"❌ Erreur SQLite (électrique): {e}")
            return False
...
# ============================================================================
# SYSTÈME PRINCIPAL - DUAL STORAGE
# ============================================================================

class SystemeSurveillance:
    """Système principal de surveillance DHT22 + PZEM-004T avec DUAL STORAGE"""
    ...
    def __init__(self, 
                 ...
                 influx_db: str = 'surveillance'):
        
        print("🔧 Initialisation du Système de Surveillance DUAL STORAGE")
        print("=" * 70)
        
        # Composants - SQLite (toujours actif)
        self.db_sqlite = DatabaseManager(db_path)
        
        # Composants - InfluxDB (optionnel)
        self.db_influx = InfluxDBManager(
            host=influx_host,
            port=influx_port,
            database=influx_db
        )
        ...
        print("=" * 70)
        print("✅ Système DUAL STORAGE initialisé avec succès")
        print(f"   → SQLite: ACTIF")
        print(f"   → InfluxDB: {'ACTIF' if self.db_influx.enabled else 'INACTIF'}")
        print()
    
    def cycle_mesure(self):
        """Exécute un cycle de mesure complet avec DUAL STORAGE"""
        ...
        if mesure_env:
            # Stocker dans SQLite
            if self.db_sqlite.inserer_mesure_environnement(mesure_env):
                self.stats_stockage['sqlite_ok'] += 1
            else:
                self.stats_stockage['sqlite_erreur'] += 1
            
            # Stocker dans InfluxDB
            if self.db_influx.ecrire_mesure_environnement(mesure_env):
                self.stats_stockage['influx_ok'] += 1
            else:
                self.stats_stockage['influx_erreur'] += 1
            
            # Garder en mémoire pour le web
            with self.lock:
        ...
        if mesure_elec:
            # Stocker dans SQLite
            if self.db_sqlite.inserer_mesure_electrique(mesure_elec):
                self.stats_stockage['sqlite_ok'] += 1
            else:
                self.stats_stockage['sqlite_erreur'] += 1
            
            # Stocker dans InfluxDB
            if self.db_influx.ecrire_mesure_electrique(mesure_elec):
                self.stats_stockage['influx_ok'] += 1
            else:
                self.stats_stockage['influx_erreur'] += 1
    ...
    def _afficher_terminal(self, env: Optional[MesureEnvironnement], 
        ...
        print(f"\n{'=' * 70}")
        print(f"📊 [{heure}] MESURES EN TEMPS RÉEL - DUAL STORAGE")
        print(f"{'=' * 70}")
        ...
        # Stats de stockage
        print(f"\n💾 STOCKAGE:")
        print(f"   ├─ SQLite  : ✅ {self.stats_stockage['sqlite_ok']:4d} | ❌ {self.stats_stockage['sqlite_erreur']:4d}")
        print(f"   └─ InfluxDB: ✅ {self.stats_stockage['influx_ok']:4d} | ❌ {self.stats_stockage['influx_erreur']:4d}")
        
        print(f"{'=' * 70}")
    
    def boucle_surveillance(self):
        """Boucle principale de surveillance"""
        print("\n🔍 Démarrage de la surveillance DUAL STORAGE...")
    ...
    def arreter(self):
        ...
        print(f"   SQLite  : {self.stats_stockage['sqlite_ok']} écritures réussies, "
              f"{self.stats_stockage['sqlite_erreur']} erreurs")
        print(f"   InfluxDB: {self.stats_stockage['influx_ok']} écritures réussies, "
              f"{self.stats_stockage['influx_erreur']} erreurs")
        
        # Fermer InfluxDB
        self.db_influx.close()
    ...
    def obtenir_donnees_dashboard(self) -> Dict:
        ...
        with self.lock:
            return {
                ...
                'statistiques': self.db_sqlite.obtenir_statistiques(heures=24),
                'stats_stockage': self.stats_stockage.copy(),
                'influxdb_actif': self.db_influx.enabled,
                'timestamp': datetime.now().isoformat()
            }
...
def api_statistiques():
    ...
    if systeme:
        return jsonify({
            'stats_24h': systeme.db_sqlite.obtenir_statistiques(heures=24),
            'stats_7j': systeme.db_sqlite.obtenir_statistiques(heures=24*7),
            'stats_stockage': systeme.stats_stockage
        })
    return jsonify({'error': 'Système non initialisé'}), 503
...
def api_status():
    ...
    if systeme:
        return jsonify({
            'sqlite': True,
            'influxdb': systeme.db_influx.enabled,
            'influxdb_ping': systeme.db_influx.test_connexion(),
            'stats': systeme.stats_stockage
        })
    return jsonify({'error': 'Système non initialisé'}), 503
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print(" 🚀 SYSTÈME DE SURVEILLANCE DHT22 + PZEM-004T - DUAL STORAGE")
    print("="*70 + "\n")
    ...
    INFLUX_PORT = 8086
    INFLUX_DB = 'surveillance'
    
    # Créer le système avec DUAL STORAGE
